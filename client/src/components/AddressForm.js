...
import { asyncCreateAddress, asyncgetDetailsByPincode } from "../redux/actions/addressAction";
...
const AddressForm = (props) => {
    ...
    const handleSubmit = (data) => {
        ...
        if (!pincodeError && city.length > 0 && state.length > 0) {
            dispatch(asyncCreateAddress({ ...data, pincode, city, state, userRef }, notify, handleClose))
        }
    }
    return (
        <Grid px={2} sx={{ px: 2, maxHeight: "80vh", mb: 5 }}>
